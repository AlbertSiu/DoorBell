...
void chk_reset(void)
{
  ...
  if (connect_before)
    {
       if (wake_up_time>7200000) shouldreboot=true;  //2 hr reboot once
    }
   else
    {
       if (wake_up_time>1800000) shouldreboot=true;  //Half hr reboot once
    }
}
...
if (connect_before)
   {
    ...
    Serial.println(WiFi.localIP());
      ...
      server.on("/", HTTP_GET, []() {
          server.sendHeader("Connection", "close");
          server.send(200, "text/html", indexhtml); // send
      });
...
void loop() {
  ...
  //*************************
  if (connect_before) server.handleClient(); //haandle server request
